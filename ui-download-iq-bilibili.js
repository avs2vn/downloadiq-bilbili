  ...
  ajaxHook({
    open: function([method, url], xhr) {
      if (url.search(/\.m3u8(\?|$)/) > -1) {
        ...
        var html = `<a href="${url}" class="remote">bấm chuột để tải xuống</a>`;
...
function updateModal({title, content}) {
  ...
  storage.downloadBtn = $.create('div', {
    id: 'dl-btn', 
    innerHTML: '<span>Tải xuống<br>video</span>',
    appendToBody: true,
  });
    ...
    GM_xmlhttpRequest({
      ...
      onload: function(res) {
        if (res.response == 'success') {
          ...
          reject('Create task fail');
        }
      },
      ontimeout: function() {
        reject('Please run first "python daemon.py"');
      }
    });
  });
}
